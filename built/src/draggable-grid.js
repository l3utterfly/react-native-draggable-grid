...
var DraggableGrid = function (props) {
    ...
    function onHandMove(_, gestureState) {
        ...
        if (activeItemIndex != closetItemIndex) {
            ...
            items.forEach(function (item, index) {
                ...
                if (index === activeItemIndex)
                    return;
                var itemPosition = blockPositions[orderMap[item.key].order];
                var itemRect = {
                    left: itemPosition.x,
                    top: itemPosition.y,
                ...
                var overlapX = Math.max(0, Math.min(dragRect_1.right, itemRect.right) - Math.max(dragRect_1.left, itemRect.left));
                var overlapY = Math.max(0, Math.min(dragRect_1.bottom, itemRect.bottom) - Math.max(dragRect_1.top, itemRect.top));
                var overlapArea = overlapX * overlapY;
                if (overlapArea > maxOverlap_1) {
                    maxOverlap_1 = overlapArea;
                    currentHoverKey = item.key;
                }
            });
            // Only consider it a hover if the overlap is at least 10% of the block area
            var minOverlapThreshold = blockWidth * blockHeight * 0.3;
            if (maxOverlap_1 < minOverlapThreshold) {
        ...
        if (hoverTargetKeyRef.current !== currentHoverKey) {
            ...
            if (hoverTargetKeyRef.current !== undefined) {
                var prevKey = (_a = items.find(function (item) { return item.key === hoverTargetKeyRef.current; })) === null || _a === void 0 ? void 0 : _a.key;
                if (prevKey === undefined)
                    return;
                react_native_1.Animated.spring(getHoverScale(prevKey), {
                    toValue: 1,
                    friction: 5,
            ...
            if (currentHoverKey !== undefined) {
                var newKey = (_b = items.find(function (item) { return item.key === currentHoverKey; })) === null || _b === void 0 ? void 0 : _b.key;
                if (newKey === undefined)
                    return;
                react_native_1.Animated.spring(getHoverScale(newKey), {
                    toValue: 1.1,
                    friction: 5,
                    useNativeDriver: false,
                }).start();
    ...
    function onHandRelease() {
        ...
        if (hoverTargetKeyRef.current !== undefined) {
            var hoverKey = (_a = items.find(function (item) { return item.key === hoverTargetKeyRef.current; })) === null || _a === void 0 ? void 0 : _a.key;
            if (hoverKey === undefined)
                return;
            react_native_1.Animated.spring(getHoverScale(hoverKey), {
                toValue: 1,
                friction: 5,
                useNativeDriver: false,
            }).start();
